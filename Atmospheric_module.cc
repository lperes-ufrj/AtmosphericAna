  ...
  if (fSaveGeantInfo)
  {
    ...
    fnGeantParticles_Primaries = 0;
      for (size_t i = 0; i < MCParticleObjs.size(); i++)
      {
        ...
        if (MCParticleObj.Process() != "primary"){ continue;}
...
*/
 ...
 // m_AtmTree->Branch("TruePrimaryBDMVertex", &fPrimaryBDMVertex);
  ...
  m_AllEvents->Branch("MCPrimaryNuPDG", &fMCPrimaryNuPDG);
  m_AllEvents->Branch("MCNuMomentum", &fMCNuMomentum);
  m_AllEvents->Branch("MCInitialPositionNu", &fMCInitialPositionNu);
...
int atm::Atmospheric::Topology(std::vector<int> fMCPdgCode)
{
  int ProtonCnt = -1;
  int PionPCnt = -1;
  int PionNCnt = -1;
